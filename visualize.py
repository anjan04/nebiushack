...
def load_experiments():
    ...
    for path in sorted(glob.glob(os.path.join(LOG_DIR, "experiment_*.json"))):
        try:
            ...
            score = data.get("primary_score") or data.get("metrics", {}).get("primary_score") or data.get("score")
            if exp_num is None or score is None:
                continue
            kept = data.get("kept", data.get("improved", False))
            if isinstance(kept, str):
                kept = kept == "improvement"
            decision = data.get("decision", "")
            if decision:
                kept = decision == "improvement"
            experiments.append({
